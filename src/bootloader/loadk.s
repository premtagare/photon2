...
.done:
	...
	;copy kernel from 0x20000 to 0x500000
...
.lessthanseg:
	...
	sub dx,1
	mov edi,0x00000000
	mov edi,0x00500000		; base address of kernel to be loaded
.nextblock:
	cmp dx,0
	je .afterblockcomp
	sub dx,1
...
bits 32
protmode:
	; Load the Kernel binary to exact location by parsing ELF file.
        mov edi,0x00500000		; Kernel ELF image is placed at this location
	mov eax,24			; Kernel Entry point is at 24th location in ELF header
	add edi,eax
	mov eax,[edi]
	mov [kernelEntryPoint],eax
	add edi,4
	mov eax,[edi]
	mov [programHeaderOffset],eax
	add edi,14
	mov ax,[edi]
	mov [sizeOfProgHeader],ax
	add edi,2
	mov ax,[edi]
	mov [numOfProgHeaders],ax
	mov edi,0x00500000
	mov eax,[programHeaderOffset]
	add edi,eax
...
.LoadProgHeader:
	...
	jne .nextProgHeader
	mov ebx,0x00500000
	mov eax,[edi+4]			; Offset in ELF binary
	add ebx,eax
	mov edx,[edi+8]			; Virtual address
...
.nextProgHeader:
	mov eax,0x00000000
	mov ax,[sizeOfProgHeader]
	add edi,eax
	mov ax,[numOfProgHeaders]
	dec ax
	mov [numOfProgHeaders],ax
	cmp ax,0
	jg .LoadProgHeader
...
programHeaderOffset 	dd 0x00500034
sizeOfProgHeader 	dw 0x0020
numOfProgHeaders	dw 0x0000
